...
namespace Deelmate\Themebee\Providers;

use App;
use File;
use Illuminate\Support\ServiceProvider;
use Deelmate\Themebee\Console\ThemeListCommand;
use Deelmate\Themebee\Contracts\ThemeContract;
use Deelmate\Themebee\Managers\Theme;

class ThemebeeServiceProvider extends ServiceProvider
{
    /**
    ...
    public function register()
    {
        ...
        $this->loadViewsFrom(__DIR__.'/../Views', 'themebee');
    }
    ...
    public function registerMiddleware()
    {
        if (config('theme.types.enable')) {
            ...
            foreach ($themeTypes as $middleware => $themeName) {
                $this->app['router']->aliasMiddleware($middleware, '\Deelmate\Themebee\Middleware\RouteMiddleware:'.$themeName);
            }
        }
    }
    ...
    public function registerThemeGeneratorCommand()
    {
        $this->app->singleton('theme.create', function ($app) {
            return new \Deelmate\Themebee\Console\ThemeGeneratorCommand($app['config'], $app['files']);
        });
    }
